...
| `/wiki-ingest <path\|folder>` | Enqueues a path under `raw/` or a configured source repo, then ingests it in one step, skipping detection. A folder is a filesystem walk, so every file under it is enqueued, tracked or not, except paths matching the `ignore:` globs (see below); an explicitly named single file is always enqueued, even if it matches an ignore rule. Use this for `raw/` drops or specific files. Give the path relative to the wiki repo root (for example, `raw/specs/api.pdf`) or as an absolute path. A path outside `raw/` and the configured source repos is rejected. To enqueue a drop and then ingest it into the wiki under a per-phase budget, see `/wiki-queue <path\|folder>` followed by `/wiki-ingest N`. |
...
| `/wiki-queue` | Shows pending extract and synth counts per source. Use it to check what's queued before or after a run. |
| `/wiki-queue all` | Forces a full detection pass (Jira and repos) and enqueues what changed, without ingesting anything into the wiki. Use it when you know new work landed and want it queued now, rather than waiting for `/wiki-ingest` to detect on an empty queue. |
...
| `/wiki-queue <path\|folder>` | Enqueues a path under `raw/` or a configured source repo without ingesting it into the wiki. A folder is a filesystem walk, so every file under it is enqueued, tracked or not, except paths matching the `ignore:` globs (see below); an explicitly named single file is always enqueued, even if it matches an ignore rule. Use this for `raw/` drops or specific files. Give the path relative to the wiki repo root (for example, `raw/specs`) or as an absolute path. A path outside `raw/` and the configured source repos is rejected. Use it to stage several `raw/` drops before a single `/wiki-ingest`. |
| `/wiki-queue backfill <repo> …` | Enqueues the repo's git-tracked files via `git ls-files`, so gitignored files and `.git/` internals are excluded; paths matching the `ignore:` globs (see below) are filtered out too (unlike the `<path\|folder>` forms, which walk the filesystem and include untracked files). Name a repo by its `wiki.config.yaml` source name (for example, `my-project`) or by path, and pass several to backfill more than one at once. At least one repo is required; there is no bare "backfill everything" form. Use it once when first adding a repo, since incremental detection enqueues nothing for an already-current clone. |
...
Detection, `backfill`, and folder enqueues all skip paths matching the `ignore:` globs — a
built-in set of junk defaults (`node_modules/` and `vendor/` trees, minified/bundle/map
output, `*.d.ts`, lockfiles, binary assets, styling, certs) plus any patterns in your
`wiki.config.yaml` `ignore:` list — so build output and vendored code never enter the queue.
An explicitly named single file is exempt: passing it directly always enqueues it, even if it
matches an ignore rule. Use `--dry-run` to preview the kept-vs-ignored split before queueing.
